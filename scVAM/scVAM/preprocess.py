...
#TODO: Fix this
# AnnSequence:作用将一个大的 matrix 划分为多个批次，并允许通过索引访问这些批次。同时，它还保存了与每个批次相关联的大小因子
class AnnSequence:
...
# read_dataset：函数是一个用于预处理单细胞分析数据集的实用工具。它根据输入参数对数据进行读取、复制、转置和分割，并执行一些数据验证操作
...
# 主要目的是对单细胞测序数据中的每个细胞的基因表达量进行 CLR 归一化处理。它首先保存原始数据，然后可能进行一些预处理（如使用 scanpy 的 normalize_per_cell 函数），接着计算每个细胞的大小因子，并最终应用 CLR 归一化。归一化后的数据存储在 adata.X 中，并返回整个处理过的 adata 对象
def clr_normalize_each_cell(adata):
    """Normalize count vector for each cell, i.e. for each row of .X"""
    # seurat_clr：是一个内部函数，用于实现 CLR（Centered Log-Ratio）归一化
    def seurat_clr(x):
