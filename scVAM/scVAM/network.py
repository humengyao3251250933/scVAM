...
class scEMC(nn.Module):
    ...
    def pretrain_autoencoder(self, X1, X_raw1, sf1, X2, X_raw2, sf2,
        ...
        num_batch = int(math.ceil(1.0 * X1.shape[0] / batch_size))
        dataset = TensorDataset(torch.Tensor(X1), torch.Tensor(X_raw1), torch.Tensor(sf1), torch.Tensor(X2),
                                torch.Tensor(X_raw2), torch.Tensor(sf2))
        ...
        for epoch in range(epochs):
            ...
            for batch_idx, (x1_batch, x_raw1_batch, sf1_batch, x2_batch, x_raw2_batch, sf2_batch) in enumerate(
                ...
                zbatch, z_num, lqbatch, mean1_tensor, mean2_tensor, disp1_tensor, disp2_tensor, pi1_tensor, pi2_tensor = self.forward(
                    x1_tensor, x2_tensor)
                recon_loss1 = self.zinb_loss(x=x_raw1_tensor, mean=mean1_tensor, disp=disp1_tensor, pi=pi1_tensor,
                                             scale_factor=sf1_tensor)
                recon_loss2 = self.zinb_loss(x=x_raw2_tensor, mean=mean2_tensor, disp=disp2_tensor, pi=pi2_tensor,
            ...
            if epoch % self.t == 0:
                print(
                    'Pretrain epoch {}, Total loss:{:.6f}, ZINB loss1:{:.6f}, ZINB loss2:{:.6f}, KL loss:{:.6f}'.format(
    ...
    def fit(self, X1, X_raw1, sf1, X2, X_raw2, sf2, lam1, lam2, y=None, lr=0.001, n_clusters=16,
        ...
        for epoch in range(num_epochs):
            ...
            for batch_idx in range(num_batch):
                ...
                recon_loss1 = self.zinb_loss(x=rawinputs1, mean=mean1_tensor, disp=disp1_tensor, pi=pi1_tensor,
                                             scale_factor=sfinputs1)
                recon_loss2 = self.zinb_loss(x=rawinputs2, mean=mean2_tensor, disp=disp2_tensor, pi=pi2_tensor,
